...
filename = '/home/yeshi/projects/aruco_markers/data/zturn.txt';
...
% % %The confusion is pose is that the gazebo camera sensor link is not as
% % %shown in gazebo but as considered for a standard camera system
% % RotM(1).R = rvec_to_R(ang(1,:));
% % camera_T_marker(1).H = [RotM(1).R,lin(1,:)'];
% % camera_T_marker(1).H = [camera_T_marker(1).H; 0 0 0 1];
% % marker_T_camera(1).H = [RotM(1).R.', -RotM(1).R.'*lin(1,:)';0 0 0 1];
% % eye_check = marker_T_camera(1).H*camera_T_marker(1).H;
% % %camera_T_marker(i).H = world_T_camera*inv(camera_T_marker(i).H);
% %  world_T_marker(1).H = world_T_camera*camera_T_marker(1).H;
% %  
% % %  figure(4)
% % % grid on;
% % % view([90,90,90])
...
% % % hold on;
% % 
% %   R = world_T_marker(1).H(1:3,1:3);
% %   P = world_T_marker(1).H(1:3,4)';
% %  
% % x(1) = P(1,1);
% % y(1) = P(1,2);
% % z(1) = P(1,3);
