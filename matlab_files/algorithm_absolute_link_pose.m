...
for i=1:1:size(lin_left,1)
    com_left(i,:) = lin_left(i,:)+[0,0.15,0];
end
...
for i = 1:1:size(ang_left,1)
    ...
    left_ft_pos(i).P = lin_left(i,:) + [0, 0.275, 0];
...
for i = 1:1:size(ang_right,1)
    ...
    right_ft_pos(i).P = lin_right(i,:) + [0.275*sin(theta(4)), 0.275*cos(theta(4)), 0];
...
for i = 1:1:size(world_T_right,2)
    ...
    %%Setting values below a threshold to zero
%     RotM12(i).R (find(RotM12(i).R  < (10^-3))) = 0;
%     lin12(i,find(lin12(i,:) < (10^-10)))=0;
...
%%Plotting Velocities
figure; 
V_plt1 = subplot(3,1,1); plot(time,V_right_mes); legend('d_x','d_y','d_z','d_{ox}','d_{oy}','d_{oz}'); title('Vel_{right measured}');
V_plt2 = subplot(3,1,2); plot(time,R_V_right_computed); legend('d_x','d_y','d_z','d_{ox}','d_{oy}','d_{oz}'); title('Revolute Vel_{right computed}');
V_plt3 = subplot(3,1,3); plot(time,P_V_right_computed); legend('d_x','d_y','d_z','d_{ox}','d_{oy}','d_{oz}'); title('Prismatic Vel_{right computed}');
linkaxes([V_plt1 V_plt2 V_plt3],'xy')

% % %%Plotting Accelerations
figure;
A_plt1 = subplot(3,1,1); plot(time,A_right_mes); legend('d_x','d_y','d_z','d_{ox}','d_{oy}','d_{oz}'); title('Acc_{right measured}');
A_plt2 = subplot(3,1,2); plot(time,R_A_right_computed); legend('d_x','d_y','d_z','d_{ox}','d_{oy}','d_{oz}'); title('Revolute Acc_{right computed}');
A_plt3 = subplot(3,1,3); plot(time,P_A_right_computed); legend('d_x','d_y','d_z','d_{ox}','d_{oy}','d_{oz}'); title('Prismatic Acc_{right computed}');
...
figure; 
w_plt1 = subplot(3,1,1); plot(time,world_ft_left); legend('e_{ox}','e_{oy}','e_{oz}','e_x','e_y','e_z'); title('Left External Wrench_{world}');
w_plt2 = subplot(3,1,2); plot(time,world_ft_right); legend('e_{ox}','e_{oy}','e_{oz}','e_x','e_y','e_z'); title('Right External Wrench_{world}');
w_plt3 = subplot(3,1,3); plot(time,dH_mes_abs); legend('e_{ox}','e_{oy}','e_{oz}','e_x','e_y','e_z'); title('Total External Wrench_{world}');
linkaxes([w_plt1 w_plt2 w_plt3],'xy')

%%Plotting Joint Hypothesis RoM
figure; 
RH_plt = subplot(2,1,1); plot(time,R_dH); legend('d_x','d_y','d_z','d_{ox}','d_{oy}','d_{oz}'); title('Revolute Hypothesis - Rate of Change in Momentum');
PH_plt = subplot(2,1,2); plot(time,P_dH); legend('d_x','d_y','d_z','d_{ox}','d_{oy}','d_{oz}'); title('Prismatic Hypothesis - Rate of Change in Momentum');
